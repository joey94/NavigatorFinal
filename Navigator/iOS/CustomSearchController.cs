...
namespace Navigator.iOS
{
    public class CustomSearchController
    {
        ...
        private readonly UITableView _searchPredictionTable;
        private readonly TableSource tableSource;

        public CustomSearchController(ViewController owner, UISearchBar searchBar, UITableView searchPredictionTable,
            List<Room> rooms)
        {
            _searchBar = searchBar;
            _searchPredictionTable = searchPredictionTable;
            owner.InvokeOnMainThread(delegate { _searchPredictionTable.Alpha = 0; });
            ...
            _searchBar.TextChanged += (sender, e) =>
            {
                ...
                {
                    tableSource.tableItems =
                        rooms.FindAll(room => room.Name.ToLower().Contains(e.SearchText.ToLower())).ToArray();
