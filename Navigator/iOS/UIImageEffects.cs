...
namespace Navigator.iOS
{
    public static class UIImageEffects
    {
        public static UIImage ApplyLightEffect(this UIImage self)
        {
            ...
            return ApplyBlur(self, 30, tintColor, 1.8f, null);
        }
        ...
        public static UIImage ApplyExtraLightEffect(this UIImage self)
        {
            ...
            return ApplyBlur(self, 20, tintColor, 1.8f, null);
        }
        ...
        public static UIImage ApplyTintEffect(this UIImage self, UIColor tintColor)
        {
            ...
            return ApplyBlur(self, 10, effectColor, -1, null);
        }

        public static UIImage ApplyBlur(UIImage image, float blurRadius, UIColor tintColor, float saturationDeltaFactor,
            UIImage maskImage)
        {
            if (image.Size.Width < 1 || image.Size.Height < 1)
            {
                Debug.WriteLine(@"*** error: invalid size: ({0} x {1}). Both dimensions must be >= 1: {2}",
                    image.Size.Width, image.Size.Height, image);
                return null;
            }
            ...
            var hasBlur = blurRadius > float.Epsilon;
            var hasSaturationChange = Math.Abs(saturationDeltaFactor - 1) > float.Epsilon;
            ...
            if (hasBlur || hasSaturationChange)
            {
                ...
                if (hasBlur)
                {
                    ...
                    var radius = (uint) (Math.Floor(inputRadius*3*Math.Sqrt(2*Math.PI)/4 + 0.5));
                    if ((radius%2) != 1)
                        radius += 1;
                    vImage.BoxConvolveARGB8888(ref effectInBuffer, ref effectOutBuffer, IntPtr.Zero, 0, 0, radius,
                        radius, Pixel8888.Zero, vImageFlags.EdgeExtend);
                    vImage.BoxConvolveARGB8888(ref effectOutBuffer, ref effectInBuffer, IntPtr.Zero, 0, 0, radius,
                        radius, Pixel8888.Zero, vImageFlags.EdgeExtend);
                    vImage.BoxConvolveARGB8888(ref effectInBuffer, ref effectOutBuffer, IntPtr.Zero, 0, 0, radius,
                        radius, Pixel8888.Zero, vImageFlags.EdgeExtend);
                }
                ...
                if (hasSaturationChange)
                {
                    ...
                    var floatingPointSaturationMatrix = new[]
                    {
                    ...
                    for (var i = 0; i < saturationMatrix.Length; i++)
                        saturationMatrix[i] = (short) Math.Round(floatingPointSaturationMatrix[i]*divisor);
                    if (hasBlur)
                    {
                        vImage.MatrixMultiplyARGB8888(ref effectOutBuffer, ref effectInBuffer, saturationMatrix, divisor,
                            null, null, vImageFlags.NoFlags);
                        effectImageBuffersAreSwapped = true;
                    }
                    else
                        vImage.MatrixMultiplyARGB8888(ref effectInBuffer, ref effectOutBuffer, saturationMatrix, divisor,
                            null, null, vImageFlags.NoFlags);
                }
